...
async function actualizarDolar() {
    try {
        ...
        console.log(
            `[${new Date().toLocaleString()}] Dólar actualizado`
        );
    } catch (err) {
...
async function actualizarTasas() {
    try {
        ...
        for (const banco of data) {
            ...
            if (banco.tnaClientes == null) continue;
            ...
            await db.query(
                ...
                `,
                [banco.entidad, banco.tnaClientes]
            );

            const [rows] = await db.query(
                'SELECT id FROM bancos_tasas WHERE nombre = ?',
                [banco.entidad]
            );
            ...
            if (rows.length > 0) {
                await db.query(
                    ...
                    ON DUPLICATE KEY UPDATE
                        tna = VALUES(tna)
                    `,
                    [rows[0].id, banco.tnaClientes]
                );
            }
        }

        console.log(
            `[${new Date().toLocaleString()}] Tasas BCRA actualizadas: ${data.length} bancos`
        );
